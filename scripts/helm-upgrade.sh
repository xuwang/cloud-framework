...
###############################################################################
# Do a helm upgrade with a values file processed by gomplate
###############################################################################
...
usage() {
    echo "${0} USAGE:"
    echo 
    echo 'upgrade "local" chart ${COMMON}/charts/my-chart'
    ...
    echo 
    echo 'upgrade to latest version of "global" chart ${FRAMEWORK_DIR}/helm-charts/my-chart/latest'
    echo '   # if ${COMMON}/charts/my-chart DOES NOT exist'
    echo '   export HELM_CHART=my-chart'
    echo '   helm-upgrade.sh'
    echo 
    echo 'upgrade to specific version of "global" chart ${FRAMEWORK_DIR}/helm-charts/my-chart/0.1.0'
    echo '   # if ${COMMON}/charts/my-chart DOES NOT exist'
    echo "   export HELM_CHART=my-chart"
    echo "   export HELM_CHART_VERSION=0.1.0"
    echo "   helm-upgrade.sh"
    echo
    echo 'upgrade multiple "local" or "global" (latest version) charts'
    echo '   # will check ${COMMON}/charts first, then ${FRAMEWORK_DIR}/helm-charts'
    echo "   helm_chart=my-chart ./helm-upgrade.sh"
    echo "   helm_chart=my-other-chart ./helm-upgrade.sh"
    echo
    echo 'debug the values passed to your chart via $helm_values_template'
    echo "   debug_gomplate=true helm_chart=my-chart ./helm-upgrade.sh"
    echo
    echo 'do a helm --dry-run'
    echo "   debug_helm=true helm_chart=external-dns ./helm-upgrade.sh"
    echo
...
# instead, the script can be used multiple times within a project, allowing upgradeation of several charts
...
echo "upgrading chart $helm_chart_path"
cat $helm_values_template | gomplate.sh | \
    $HELM ${extra_global_args} \
        upgrade -f - ${helm_release_name} \
        ${extra_cmd_args} \
        ${helm_chart_path} \
        --namespace "${helm_namespace}"